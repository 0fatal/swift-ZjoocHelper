...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private lazy var contentView: NSView? = (statusItem.value(forKey: "window") as? NSWindow)?.contentView
    
    private func setupStatusItem() {
        statusItem = NSStatusBar.system.statusItem(withLength: 20)
        if let button = statusItem.button {
            button.image = NSImage(named: "favicon")
    ...
    private func setupMenus() {
        ...
        ZjoocVM.shared.hostingView = NSHostingView(rootView: AnyView(MenuView(vm: MenuVM.shared)))
